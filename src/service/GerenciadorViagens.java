...
public class GerenciadorViagens {
    ...
    public void validarEIniciarViagem(Veiculo veiculo, Motorista motorista,
            double distanciaDestino) {

        double autonomiaDisponivel = veiculo.calcularAutonomia();

        PrintUtil.imprimir("Verificando frota...");
        System.out.println("Veículo placa [" + veiculo.getPlaca() + "] tem " + autonomiaDisponivel
                + " km de autonomia.");

        if (distanciaDestino > autonomiaDisponivel) {
            throw new CombustivelInsuficienteException("Combustível insuficiente! O destino fica a "
                    + distanciaDestino + " km, mas o veiculo só consegue rodar mais "
                    + autonomiaDisponivel + " km de distância");
        }
        ...
        System.out.println("Motorista " + motorista.getNome() + " já pode partir rumo ao destino ("
                + distanciaDestino + " km).");
    }
